...
dates = '13.11.2022'
cookie = 'ssuz_sessionid=58y9vdyy1whu0hcel3wjmv4s96gcov0h'
...
spisok = [
    ...
    {"group": "тис_исп320р", "student_id": 61712, "group_id": 4086,"subject_id":"6190", "id":7},#7
    {"group": "ркис_исп320рв", "student_id": 61680, "group_id": 4087,"subject_id":"6189", "id":8},#8
...
def saveThemesTeory(diir):
  ...
  for name in spisok:
    ...
    with open(diir+"\\rasp_"+name['group']+'.txt', encoding="utf-8") as file:
...
def saveThemesPracticy(diir):
  for el in spisok_practicy:
      ...
      # print(rows)
      # return 0
      with open(diir+"\\rasp_"+el['group']+'.txt', encoding="utf-8") as file:
      # считываем строчки из файла
        try:
          lines = file.readlines()
          for i in range(len(lines)):
