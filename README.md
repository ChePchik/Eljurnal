...
Запустить приложение `TemaEJ.py` Выбрать пункт 0. Ввести свой логин и пароль.
**Логин и пароль будут передаваться в открытом виде по http.**
_Если для вас критично это, то есть второй способ._

После авторизации если выдало 200 и символы ниже, то отлчно,не закрываем приложение и идём дальше.

**Для тех кто шарит есть второй способ**
Смотри пункт 4
...
4. Заполнение файла `.env`. Если не помним поднимаемся до `Подготовка, делаем один раз` и ниже.

__Если всё заполнили и дальше вам не понятно, значит пропускаем, это тем кто шарит__
